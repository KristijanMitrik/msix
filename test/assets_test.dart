...
void main() {
  ...
  test('copy defaults icons', () async {
    await File(p.join(tempFolderPath, 'icons', 'test1.png')).create(recursive: true);
    await File(p.join(tempFolderPath, 'icons', 'test2.png')).create(recursive: true);
    ...
    expect(await File(p.join(tempFolderPath, 'Images', 'test1.png')).exists(), true);
    expect(await File(p.join(tempFolderPath, 'Images', 'test2.png')).exists(), true);
  });

  test('generate icons (expect 82 new images)', () async {
    Image image = Image(width: 320, height: 240);
    fill(image, color: ColorRgb8(0, 0, 255));
    await File(p.join(tempFolderPath, 'test.png')).writeAsBytes(encodePng(image));
    await Directory(p.join(tempFolderPath, 'Images')).create(recursive: true);
    await Future.delayed(const Duration(milliseconds: 200));
    config.logoPath = p.join(tempFolderPath, 'test.png');
    await Assets().createIcons();
    await Future.delayed(const Duration(milliseconds: 100));
    expect((await Directory(p.join(tempFolderPath, 'Images')).list().toList()).length, 82);
    await Future.delayed(const Duration(milliseconds: 100));
  });

  test('copy vclibs files', () async {
    var vclibsFolderPath = p.join(tempFolderPath, 'VCLibs', 'x64');
    await File(p.join(vclibsFolderPath, 'msvcp140.dll')).create(recursive: true);
    await File(p.join(vclibsFolderPath, 'vcruntime140_1.dll')).create(recursive: true);
    await File(p.join(vclibsFolderPath, 'vcruntime140.dll')).create(recursive: true);
    await Future.delayed(const Duration(milliseconds: 100));
    config
    ...
    expect(await File(p.join(vclibsFolderPath, 'vcruntime140_1.dll')).exists(), true);
    expect(await File(p.join(vclibsFolderPath, 'vcruntime140.dll')).exists(), true);
  });

  test('copy context menu dll', () async {
    var dllFile =
        await File(p.join(tempFolderPath, 'test', 'ContextMenu.dll')).create(recursive: true);
    await Future.delayed(const Duration(milliseconds: 100));
    config.msixAssetsPath = tempFolderPath;
    await Assets().copyContextMenuDll(dllFile.path);
    await Future.delayed(const Duration(milliseconds: 100));
    expect(await File(p.join(tempFolderPath, 'ContextMenu.dll')).exists(), true);
  });
  ...
  test('clean temporary files', () async {
    ...
    await Directory(p.join(tempFolderPath, 'VCLibs', 'x64')).create(recursive: true);
    await Future.delayed(const Duration(milliseconds: 100));
    config
      ..msixAssetsPath = tempFolderPath
      ..architecture = 'x64';
    ...
    await Future.delayed(const Duration(milliseconds: 100));
    expect(await File(p.join(tempFolderPath, 'resources.scale-125.pri')).exists(), false);
